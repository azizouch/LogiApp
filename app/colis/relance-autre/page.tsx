...
export default function ColisRelanceAutrePage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // Build query for relanced colis for other clients
        // Note: In a real implementation, you would have a field to identify relanced colis for other clients
        // For now, we're just simulating with a query that would get such colis
        ...
        .eq('statut', 'En attente') // Assuming relanced colis have a specific status
        // You might add a specific condition for relanced colis for other clients here
        // .eq('is_relanced_other_client', true)
    ...
    <div className="container mx-auto p-6">
      <div className="mb-8">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold">Relancé Autre Client</h1>
            <p className="text-muted-foreground mt-2">
              Colis qui ont été relancés pour un autre client et qui sont en attente de livraison
            </p>
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white rounded-lg shadow-sm border p-6">
          <UserCircle className="mx-auto h-12 w-12 text-muted-foreground opacity-50 mb-4" />
          <h3 className="text-lg font-medium mb-2">Aucun colis relancé pour autre client</h3>
          <p className="text-muted-foreground">
            Vous n'avez actuellement aucun colis relancé pour un autre client à livrer.
          </p>
        </div>
      ) : (
