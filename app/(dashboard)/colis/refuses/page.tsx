...
export default function ColisRefusesPage() {
  const [colisRefuses, setColisRefuses] = useState<any[]>([])
    ...
    const fetchColisRefuses = async () => {
      try {
        ...
        const formattedColis = colisData.map(colis => ({
          ...
          date_refus: colis.date_mise_a_jour ? new Date(colis.date_mise_a_jour).toLocaleDateString('fr-FR') : 'Date inconnue',
        ...
        setColisRefuses(formattedColis)
        ...
        console.error('Error fetching refused colis:', JSON.stringify(error))
  ...
  useEffect(() => {
    ...
    if (selectedDate) {
      ...
      filtered = filtered.filter(colis => colis.date_refus === dateString)
    }
  ...
  }, [colisRefuses, searchQuery, selectedClient, selectedLivreur, selectedMotif, selectedDate])
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-3xl font-bold">Colis Refusés</h1>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-center text-muted-foreground py-4">Aucun colis refusé trouvé</p>
          ) : (
