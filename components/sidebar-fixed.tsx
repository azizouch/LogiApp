  ...
  },
  {
    ...
    submenuItems: [
      ...
      },
      {
        ...
        href: "/colis/livres",
        icon: PackageCheck,
        roles: ["Admin", "Gestionnaire"], // Admin and Gestionnaire can access delivered colis
      },
      {
        title: "Colis Refusés",
        href: "/colis/refuses",
        icon: PackageX,
        roles: ["Admin", "Gestionnaire"], // Admin and Gestionnaire can access refused colis
      },
      {
        title: "Colis Annulés",
        href: "/colis/annules",
        icon: Package,
        roles: ["Admin", "Gestionnaire"], // Admin and Gestionnaire can access cancelled colis
      },
    ],
  },
  {
...
export function SidebarFixed() {
  ...
  return (
    <div
      ...
      onClick={(e) => {
        // Prevent clicks on the sidebar itself from closing it
        e.stopPropagation();
      }}
    >
      ...
      <nav
        ...
        onClick={(e) => {
          // Prevent clicks on the nav container from closing the sidebar
          e.stopPropagation();
        }}
      >
          ...
          <div key={item.title || item.href} className="relative group">
            {item.submenu ? (
              ...
              >
                <button
                  onClick={(e) => {
                    console.log("Submenu button clicked", item.title);
                    ...
                    // IMPORTANT: Do NOT close the sidebar when clicking on submenu toggle
                    // This is especially important on mobile

                    // Force the sidebar to stay open on mobile
                    if (isMobile) {
                      // This is a hack to ensure the sidebar stays open
                      setTimeout(() => {
                        if (!isOpen) {
                          toggle();
                        }
                    ...
                    {item.submenuItems?.map((subItem) => (
                      <Link
                        ...
                        onClick={(e) => {
                          ...
                          // This is a navigation link, so we DO want to close the sidebar
                          if (isMobile) {
